  ...
  return (
    <div className="flex w-full flex-row justify-between">
      <span className="space-x-1 text-lg font-semibold tracking-wide">
        It is{' '}
        <span
          className={cn('font-bold uppercase', props.currentPlayer === 'red' ? 'text-red-500' : 'text-neutral-400')}
        >
          {props.currentPlayer}
        </span>
        &apos;s turn
      ...
      <span className="flex flex-row gap-2">
        ...
        <span>
          <span className="font-bold uppercase text-neutral-500">Black:</span> {props.scores.get('black') ?? 0}
        </span>
      </span>
...
function AvailableMove({ children }: PropsWithChildren) {
  return (
    <div className={cn('flex h-full w-full items-center justify-center', 'bg-green-400')}>
      <button className="flex h-full w-full items-center justify-center hover:bg-green-500">{children}</button>
    </div>
  );
}

function AvailableCapture({ children }: PropsWithChildren) {
  return (
    <div className={cn('flex h-full w-full items-center justify-center', 'bg-red-400')}>
            ...
            return (
              <>
                ...
                {match({
                  ...
                  .otherwise(() => (
                    <BasicTile position={position}>{currentPiece ? <Piece {...currentPiece} /> : null}</BasicTile>
                  ))}
              </>
            );
          })
